...
std::string IR_filename_ONNX = "D:\\OpenVINO\\model\\yolo\\yolov4v1.onnx";

string images_path = "D:\\OpenVINO\\model\\yolo\\\pic\\";
	...
	for (size_t c = 0; c < cycle; c++)
	{
		...
		for (size_t epoch_i = 0; epoch_i < epoch; epoch_i++)
		{
			...
			for (size_t batch_size_i = 0; batch_size_i < batch_size; batch_size_i++)
			{
				...
				cv::imwrite("res\\Result" + to_string(epoch_i) + "_" + to_string(batch_size_i) + ".jpg", org_pic);
			}
